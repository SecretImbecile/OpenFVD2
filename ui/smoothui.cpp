...
smoothUi::smoothUi(trackHandler *_track, QWidget *parent)
    : QDialog(parent), ui(new Ui::smoothUi) {
...
void smoothUi::applyRollSmoothFilter(smoothHandler *_handler) {
  ...
  for (int i = fromNode; i < toNode; ++i) {
    ...
    if (i < fromNode + length / 2 * iter) {
      ...
      curNode = m_track->getPoint(i);
      cur->append(t * (curNode->fRollSpeed + curNode->fSmoothSpeed) +
                  t1 * firstValue + t2 * lastValue);
    }
    last->append(cur->last());
    orig.append(cur->last());
  }
...
void smoothUi::on_buttonBox_accepted() { applyRollSmooth(); }
...
void smoothUi::updateUi() {
  ...
  for (int i = 0; i < m_track->smoothList.size(); ++i) {
    ...
    if (!m_track->smoothList[i]->treeItem->parent()) {
      ui->smoothUnitTree->insertTopLevelItem(i,
                                             m_track->smoothList[i]->treeItem);
    }
  }
}
...
void smoothUi::on_smoothUnitTree_itemSelectionChanged() {
  ...
  for (i = 0; i < m_track->smoothList.size(); ++i) {
    if (m_track->smoothList[i]->treeItem == selected)
      break;
  }
...
void smoothUi::on_smoothUnitTree_itemChanged(QTreeWidgetItem *item,
                                             int column) {
  if (phantomChanges)
    return;
  phantomChanges = true;
  ...
  if (column == 6) {
    int i;
    for (i = 0; i < m_track->smoothList.size(); ++i) {
      if (m_track->smoothList[i]->treeItem == item)
        break;
    }
...
void smoothUi::on_newButton_released() {
  m_track->smoothList.append(
      new smoothHandler(m_track, -2, &customChar, 400, 1, 0, 1000));
  updateUi();
}
...
void smoothUi::on_removeButton_released() {
  int i;
  for (i = 0; i < m_track->smoothList.size(); ++i) {
    if (m_track->smoothList[i] == curHandler)
      break;
  }
  delete curHandler;
  ...
  for (int i = 0; i < m_track->smoothList.size(); ++i) {
    if (m_track->smoothList[i]->active) {
      ...
      if (cur->getFrom() > cur->getTo()) {
        str.append(QString("Warning: Smoothing item \"")
                       .append(cur->treeItem->text(1))
                       .append("\" (")
                       .append(cur->treeItem->text(0))
                       .append(") ")
                       .append("is set to begin before it ends.\n"));
      } else if (2.f * cur->getLength() > cur->getTo() - cur->getFrom()) {
        str.append(
            QString("Warning: Smoothing item \"")
                .append(cur->treeItem->text(1))
                .append("\" (")
                .append(cur->treeItem->text(0))
                .append(") ")
                .append("might be too short for its current filter length.\n"));
      }
    }
  }
