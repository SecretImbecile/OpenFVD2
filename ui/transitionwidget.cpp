...
transitionWidget::~transitionWidget() { delete ui; }
...
void transitionWidget::changeSubfunc(subfunc *_newSubfunc) {
  ...
  if (_newSubfunc->parent->secParent->type == curved) {
    ui->lengthSpin->setValue(selectedFunc->maxArgument -
                             selectedFunc->minArgument);
    ui->lengthSpin->setSuffix(QString("%1").arg(QChar(0xb0)));
  } else if (_newSubfunc->parent->secParent->bArgument == TIME &&
             _newSubfunc->parent->secParent->type != straight) {
    ui->lengthSpin->setValue(selectedFunc->maxArgument -
                             selectedFunc->minArgument);
    ui->lengthSpin->setSuffix(QString("s"));
  } else {
    ui->lengthSpin->setValue(
        (selectedFunc->maxArgument - selectedFunc->minArgument) *
  ...
  // ToZero shenanigans
  // ---
  if ((_newSubfunc->parent->secParent->type == forced ||
       _newSubfunc->parent->secParent->type == geometric) &&
  ...
  if ((_newSubfunc->parent->secParent->type == curved ||
       _newSubfunc->parent->secParent->type == straight ||
       _newSubfunc->parent->type != funcRoll ||
       _newSubfunc->parent->secParent->bArgument == DISTANCE) &&
  ...
  float diff = selectedFunc->maxArgument -
               mParent->selTrack->trackData->activeSection->getMaxArgument();

  if (diff > 0) {
    ui->errLabel->setText(QString("Warning: Transition is ")
                              .append(QString().number(diff + 0.0001, 'f', 2))
                              .append(" longer than Section!"));
    ui->errLabel->show();
  } else {
    ui->errLabel->hide();
  }

  mParent->selTrack->mUndoHandler->oldLengthSpinValue =
      selectedFunc->maxArgument - selectedFunc->minArgument;
...
void transitionWidget::on_lengthSpin_valueChanged(double arg1) {
  ...
  const bool need_length_factor =
      (selectedFunc->parent->secParent->bArgument == DISTANCE &&
       (selectedFunc->parent->secParent->type == forced ||
        selectedFunc->parent->secParent->type == geometric)) ||
      selectedFunc->parent->secParent->type == straight;

  if (need_length_factor) {
    selectedFunc->parent->changeLength(
        arg1 / gloParent->mOptions->getLengthFactor(), index);
  } else {
  ...
  inTrack->trackData->updateTrack(
      inTrack->trackData->activeSection,
      (int)(selectedFunc->minArgument * F_HZ - 1.5f));
  ...
  if (!inTrack->mUndoHandler->busy) {
    ...
    temp->toValue =
        QVariant(selectedFunc->maxArgument - selectedFunc->minArgument);
...
void transitionWidget::on_transitionBox_currentIndexChanged(int index) {
  ...
  changeSubfunc(selectedFunc); // to make sure the frames are up to date (after
                               // undo settings because that would fuck them up)
}
...
void transitionWidget::on_quinticBox_currentIndexChanged(int index) {
  ...
  case 1:
    ui->quinticSpin->setVisible(true);
    selectedFunc->arg1 = fabs(selectedFunc->arg1) < 0.005
                             ? -ui->quinticSpin->value()
                             : -fabs(selectedFunc->arg1);
    phantomChanges = true;
    ui->quinticSpin->setValue(fabs(selectedFunc->arg1));
    phantomChanges = false;
    break;
...
subfunc *transitionWidget::getSelectedFunc() { return selectedFunc; }
...
void transitionWidget::on_appendButton_released() {
  ...
  inTrack->trackData->updateTrack(
      inTrack->trackData->activeSection,
      (int)(selectedFunc->maxArgument * F_HZ - 1.5f));
  ui->removeButton->setEnabled(true);
...
void transitionWidget::on_prependButton_released() {
  ...
  inTrack->trackData->updateTrack(
      inTrack->trackData->activeSection,
      (int)(selectedFunc->parent->funcList[atIndex + 1]->minArgument * F_HZ -
            1.5f));
  ui->removeButton->setEnabled(true);
  mParent->changeSelection(selectedFunc->parent->funcList[atIndex + 1]);
  mParent->redrawGraphs();
