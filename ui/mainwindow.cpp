...
MainWindow::MainWindow(QWidget *parent)
  ...
  mOptions->setWindowFlags(mOptions->windowFlags() &
  ...
  } else if (glView->format().majorVersion() < 3 ||
             (glView->format().majorVersion() == 3 &&
              glView->format().minorVersion() < 1)) {
    glView->legacyMode = true;
  } else {
    glView->legacyMode = false;
  }
  ...
  tabBar->tabButton(0, QTabBar::RightSide)
      ->resize(0, 0); // hide close button on project tab
  ...
  connect(this, SIGNAL(emitMessage(QString, int)), ui->statusBar,
...
void MainWindow::initProject() { ui->projectTab->init(); }
...
void MainWindow::backupSave() {
  ...
  saver *gott = new saver(QString().append(currentFileName).append(".bak"),
                          ui->projectTab, this);
  gott->doSave();
  delete gott;
  this->setWindowTitle(QString("FVD++ 0.8a [020325] - " + currentFileName));
...
void MainWindow::on_actionUseShader0_triggered() { useShader(0); }

void MainWindow::on_actionUseShader1_triggered() { useShader(1); }

void MainWindow::on_actionUseShader2_triggered() { useShader(2); }

void MainWindow::on_actionUseShader3_triggered() { useShader(3); }

void MainWindow::on_actionUseShader4_triggered() { useShader(4); }

void MainWindow::on_actionUseShader5_triggered() { useShader(5); }
...
void MainWindow::updateInfoPanel() {
  ...
  if (curTrack()->lSections.size() != 0 && curTrack()->activeSection != NULL) {
    lastnode = &curTrack()
                    ->activeSection
                    ->lNodes[curTrack()->activeSection->lNodes.size() - 1];
  } else {
...
void MainWindow::updateInfoPanel(mnode *lastnode) {
  ...
  glm::mat4 anchorBase = glm::translate(curTrack()->startPos) *
                         glm::rotate(TO_RAD(curTrack()->startYaw - 90.f),
                                     glm::vec3(0.f, 1.f, 0.f));

  glm::vec3 worldPos =
      glm::vec3(anchorBase * glm::vec4(lastnode->vPosHeart(heart), 1.f));
  ...
  if (worldPos.z >= 0.f)
    str3.prepend('+');
  ui->infoPosLabel->setText(QString("X: ")
                                .append(str1)
                                .append(mOptions->getLengthString())
                                .append(QString("  Y: "))
                                .append(str2)
                                .append(mOptions->getLengthString())
                                .append(QString("  Z: "))
  ...
  if (lastnode->fVel >= 0.f)
    str1.prepend('+');
  ui->infoSpeedLabel->setText(
      QString("Speed: ").append(str1).append(mOptions->getSpeedString()));

  str1 =
      QString::number(lastnode->getDirection() + curTrack()->startYaw, 'f', 3);
  str2 = QString::number(lastnode->getYawChange(), 'f', 3);
  if (!str1.startsWith('-'))
    str1.prepend('+');
  if (!str2.startsWith('-'))
    str2.prepend('+');
  ui->infoDirLabel->setText(QString("Yaw: ")
                                .append(str1)
                                .append(QString("%1 (").arg(QChar(0xb0)))
                                .append(str2)
                                .append(QString("%1/s)").arg(QChar(0xb0))));
  ...
  if (!str2.startsWith('-'))
    str2.prepend('+');
  ui->infoPitchLabel->setText(QString("Pitch: ")
                                  .append(str1)
                                  .append(QString("%1 (").arg(QChar(0xb0)))
                                  .append(str2)
                                  .append(QString("%1/s)").arg(QChar(0xb0))));
  ...
  if (!str2.startsWith('-'))
    str2.prepend('+');
  ui->infoRollLabel->setText(QString("Roll: ")
                                 .append(str1)
                                 .append(QString("%1 (").arg(QChar(0xb0)))
                                 .append(str2)
                                 .append(QString("%1/s)").arg(QChar(0xb0))));
  ...
  str1 =
      QString::number(lastnode->forceNormal + lastnode->smoothNormal, 'f', 3);
  str2 =
      QString::number(lastnode->forceLateral + lastnode->smoothLateral, 'f', 3);
  if (!str1.startsWith('-'))
    str1.prepend('+');
  if (!str2.startsWith('-'))
    str2.prepend('+');
  ui->infoNormalLabel->setText(
      QString("y-Accel: ").append(str1).append(QString("g")));
  ui->infoLateralLabel->setText(
      QString("x-Accel: ").append(str2).append(QString("g")));
}
...
void MainWindow::setUndoButtons() {
  ...
  if (mWidget->inTrack->mUndoHandler->stackIndex != -1 &&
      mWidget->inTrack->mUndoHandler->stackIndex !=
          mWidget->inTrack->mUndoHandler->lActions.size()) {
...
void MainWindow::on_actionConversion_Panel_triggered() { mConversion->show(); }

int MainWindow::getPovPos() { return glView->povPos; }
...
void MainWindow::openTab(trackHandler *_track) {
  ...
  if (_track->tabId == -1) {
    _track->tabId = ui->tabChooser->addTab(_track->trackWidgetItem,
                                           _track->trackData->name);
  }
...
void MainWindow::on_tabChooser_currentChanged(int index) {
  ...
  if (index) {
    ...
    for (int i = 0; i < ui->tabChooser->count(); ++i) {
      ui->tabChooser->widget(i)->setGeometry(
          0, 0, ui->tabChooser->widget(i)->width(),
          ui->tabChooser->widget(i)->height() - 150);
    }
...
void MainWindow::doAutoSave() { backupSave(); }
