...
graphWidget::graphWidget(QWidget *parent, trackHandler *_track)
  ...
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(0)->child(0), banking, selTrack,
      yAxes[0], gloParent->mOptions->rollColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(0)->child(1), rollSpeed, selTrack,
      yAxes[0], gloParent->mOptions->rollColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(0)->child(2), rollAccel, selTrack,
      yAxes[0], gloParent->mOptions->rollColor));

  pGraphList.append(
      new graphHandler(ui->selTree->topLevelItem(1)->child(1)->child(0), nForce,
                       selTrack, yAxes[1], gloParent->mOptions->normColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(1)->child(1), nForceChange, selTrack,
      yAxes[1], gloParent->mOptions->normColor));

  pGraphList.append(
      new graphHandler(ui->selTree->topLevelItem(1)->child(2)->child(0), lForce,
                       selTrack, yAxes[1], gloParent->mOptions->latColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(2)->child(1), lForceChange, selTrack,
      yAxes[1], gloParent->mOptions->latColor));

  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(3)->child(0), pitchChange, selTrack,
      yAxes[2], gloParent->mOptions->pitchColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(3)->child(1), yawChange, selTrack,
      yAxes[3], gloParent->mOptions->yawColor));

  pGraphList.append(new graphHandler(ui->selTree->topLevelItem(2)->child(0),
                                     secBoundaries, selTrack,
                                     ui->plotter->yAxis, NULL));
  pGraphList.append(new graphHandler(ui->selTree->topLevelItem(2)->child(1),
                                     povPos, selTrack, NULL, NULL));

  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(0)->child(1), smoothedRollSpeed,
      selTrack, yAxes[0], gloParent->mOptions->rollColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(1)->child(0), smoothedNForce,
      selTrack, yAxes[1], gloParent->mOptions->normColor));
  pGraphList.append(new graphHandler(
      ui->selTree->topLevelItem(1)->child(2)->child(0), smoothedLForce,
      selTrack, yAxes[1], gloParent->mOptions->latColor));
  ...
  connect(ui->plotter, SIGNAL(mousePress(QMouseEvent *)), this,
          SLOT(MousePressedPlotter()));
  connect(ui->plotter, SIGNAL(mouseMove(QMouseEvent *)), this,
          SLOT(setPlotRanges()));
  connect(ui->plotter, SIGNAL(mouseWheel(QWheelEvent *)), this,
          SLOT(MouseWheelPlotter()));

  ui->plotter->setNotAntialiasedElement(
      (QCP::AntialiasedElement)(0xFFFF ^ 0x0020), true);
  ui->plotter->setPlottingHint(QCP::phForceRepaint, true);

  connect(ui->plotter, SIGNAL(selectionChangedByUser()), this,
...
void graphWidget::on_selTree_itemChanged(QTreeWidgetItem *item, int column) {
  ...
  if (item->checkState(1) == Qt::Checked) {
    if (i % 2 && (i - 1) / 2 >= 0 && (i - 1) / 2 < 3 &&
        selTrack->trackData->smoother &&
        selTrack->trackData->smoother->active()) {
      drawGraph(11 + (i - 1) / 2);
    }
    drawGraph(i);
  } else {
...
void graphWidget::setPlotRanges() {
    ...
    for (int i = 0; i < 9; ++i) {
      ...
      if (curGraph->drawn && curGraph->usedAxis == yAxes[a]) {
        ...
        if (curGraph->graphList.size()) {
          maxKey = maxKey > curGraph->graphList.last()->getKeyRange(valid).upper
                       ? maxKey
                       : curGraph->graphList.last()->getKeyRange(valid).upper;
          minKey =
              minKey < curGraph->graphList.first()->getKeyRange(valid).lower
                  ? minKey
                  : curGraph->graphList.first()->getKeyRange(valid).lower;
        }
        for (int j = 0; j < curGraph->graphList.size(); ++j) {
          if (curGraph->graphList[j]->getKeyRange(valid).upper >
                  ui->plotter->xAxis->range().lower &&
              curGraph->graphList[j]->getKeyRange(valid).lower <
                  ui->plotter->xAxis->range().upper) {
            lowerBound =
                lowerBound < curGraph->graphList[j]->getValueRange(valid).lower
                    ? lowerBound
                    : curGraph->graphList[j]->getValueRange(valid).lower;
            upperBound =
                upperBound > curGraph->graphList[j]->getValueRange(valid).upper
                    ? upperBound
                    : curGraph->graphList[j]->getValueRange(valid).upper;
          }
        }
      }
    }
  ...
  edge =
      (ui->plotter->xAxis->range().upper - ui->plotter->xAxis->range().lower) /
      3.;
...
void graphWidget::drawGraph(int index) {
  if (pGraphList[index]->mType == povPos) {
    if (pGraphList[index]->treeItem->checkState(1) == Qt::Unchecked ||
        !glView->povMode) {
      ui->povLine->hide();
      return;
    }
  ...
  if (selTrack->trackData->activeSection) {
    pGraphList[index]->fillGraphList(
        ui->plotter->xAxis, selTrack->trackData->activeSection->bArgument,
        selTrack->trackData->activeSection->bOrientation,
        pGraphList[secBoundaries]->drawn);
  } else {
    pGraphList[index]->fillGraphList(ui->plotter->xAxis, TIME, QUATERNION,
                                     pGraphList[secBoundaries]->drawn);
  }

  for (int i = 0; i < pGraphList[index]->graphList.size(); ++i) {
    ui->plotter->addPlottable(pGraphList[index]->graphList[i]);
    subfunc *temp = (subfunc *)pGraphList[index]
                        ->graphList[i]
                        ->property("p")
                        .value<void *>();
...
bool graphWidget::drawPoVLine() {
  if (ui->povLine->isHidden())
    ui->povLine->show();
  bool changed = false;
  double coord;
  if (selTrack->trackData->activeSection &&
      selTrack->trackData->activeSection->bArgument == DISTANCE) {
  ...
  double diff = coord - ui->plotter->xAxis->range().lower * 0.8 -
                ui->plotter->xAxis->range().upper * 0.2;
  if (diff < 0 && coord > 0) {
    ui->plotter->xAxis->setRange(ui->plotter->xAxis->range().lower + diff,
                                 ui->plotter->xAxis->range().upper + diff);
    changed = true;
  }
  diff = ui->plotter->xAxis->range().upper * 0.8 +
         ui->plotter->xAxis->range().lower * 0.2 - coord;
  if (diff < 0) {
    ui->plotter->xAxis->setRange(ui->plotter->xAxis->range().lower - diff,
                                 ui->plotter->xAxis->range().upper - diff);
    changed = true;
...
void graphWidget::undrawGraph(int index) {
  ...
  while (pGraphList[index]->graphList.size())
    pGraphList[index]->graphList.removeAt(0);
}
...
void graphWidget::curSectionChanged(sectionHandler *_section) {
  ...
  ui->transitionEditor->adjustLengthSteps(_section->sectionData->type,
                                          _section->sectionData->bArgument);
  ui->tabWidget->removeTab(1);
...
bool graphWidget::changeSelection(subfunc *_sel) {
  ...
  for (int i = 0; i < ui->plotter->graphCount(); ++i) {
    if ((subfunc *)ui->plotter->graph(i)->property("p").value<void *>() ==
        _sel) {
...
void graphWidget::redrawGraphs(bool otherArgument) {
  for (int i = 0; i < pGraphList.size(); ++i) {
    if (pGraphList[i]->drawn) {
      if (i % 2 && (i - 1) / 2 >= 0 && (i - 1) / 2 < 3) {
        undrawGraph(11 + (i - 1) / 2);
        if (selTrack->trackData->smoother &&
            selTrack->trackData->smoother->active()) {
          drawGraph(11 + (i - 1) / 2);
        }
      }
  ...
  if (otherArgument) {
    ...
    if (curTrack->activeSection->bArgument == TIME) {
      ...
      if (rLower <= 0.) {
        rUpper = curTrack->getPoint(maxPoints <= rUpper ? maxPoints : rUpper)
                     ->fTotalLength;
        rLower = 0.;
      } else {
        rUpper = curTrack->getPoint(maxPoints <= rUpper ? maxPoints : rUpper)
...
void graphWidget::on_plotter_customContextMenuRequested(const QPoint &pos) {
  ...
  menu->addAction("Append Transition", ui->transitionEditor,
                  SLOT(on_appendButton_released()));
  menu->addAction("Prepend Transition", ui->transitionEditor,
                  SLOT(on_prependButton_released()));
  if (selFunc->parent->funcList.size() > 1) {
    menu->addAction("Remove Transition", ui->transitionEditor,
                    SLOT(on_removeButton_released()));
  }
...
void graphWidget::keyPressEvent(QKeyEvent *event) {
  ...
  case Qt::Key_Right:
    if (selFunc) {
      if ((i = selFunc->parent->getSubfuncNumber(selFunc)) ==
          selFunc->parent->funcList.size() - 1) {
    ...
    if (selFunc) {
      if (selFunc->parent->secParent->type == forced) {
        ...
        if (selFunc->parent->type == funcRoll) {
          ok = changeSelection(
              i < selFunc->parent->secParent->normForce->funcList.size()
                  ? selFunc->parent->secParent->normForce->funcList[i]
                  : selFunc->parent->secParent->normForce->funcList.last());
          if (!ok)
            ok = changeSelection(
                i < selFunc->parent->secParent->latForce->funcList.size()
                    ? selFunc->parent->secParent->latForce->funcList[i]
                    : selFunc->parent->secParent->latForce->funcList.last());
          ...
          if (!ok)
            ok = changeSelection(
                i < selFunc->parent->secParent->rollFunc->funcList.size()
                    ? selFunc->parent->secParent->rollFunc->funcList[i]
                    : selFunc->parent->secParent->rollFunc->funcList.last());
        } else {
          ok = changeSelection(
              i < selFunc->parent->secParent->rollFunc->funcList.size()
                  ? selFunc->parent->secParent->rollFunc->funcList[i]
...
void graphWidget::setBezPoints() {
  ...
  if (selFunc && selFunc->degree == freeform) {
    ...
    if (selFunc->parent->secParent->bArgument == TIME) {
      until =
          selTrack->trackData->getNumPoints(selFunc->parent->secParent) / F_HZ;
    } else {
    ...
    for (int i = 0; i < selFunc->pointList.size(); ++i) {
      if (bezPoints[i]->isDragged) {
        selFunc->pointList[i].x =
            (ui->plotter->xAxis->pixelToCoord(bezPoints[i]->pos().x() + 6) -
             selFunc->minArgument - until) /
            (selFunc->maxArgument - selFunc->minArgument);
        selFunc->pointList[i].y =
            (yAxis->pixelToCoord(bezPoints[i]->pos().y() + 6) -
             selFunc->startValue) /
            (selFunc->symArg);
        selFunc->updateBez();
        selTrack->trackData->updateTrack(
            selTrack->trackData->activeSection,
            (int)(selFunc->minArgument * F_HZ - 1.5f));
      } else {
        int x = x1 * (1 - selFunc->pointList[i].x) +
                x2 * selFunc->pointList[i].x - 6;
        int y = y1 * (1 - selFunc->pointList[i].y) +
                y2 * selFunc->pointList[i].y - 6;
