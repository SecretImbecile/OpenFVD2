...
trackWidget::trackWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::trackWidget) {
  ...
  // sectionHandler* newSec = new sectionHandler(track->trackData, anchor,
  // sectionList.size()); sectionList.append(newSec);
...
trackWidget::trackWidget(QWidget *parent, trackHandler *_track)
  ...
  sectionHandler *newSec =
      new sectionHandler(inTrack->trackData, anchor, sectionList.size());
...
void trackWidget::addSection(secType _type) {
  sectionHandler *newSec =
      new sectionHandler(inTrack->trackData, _type, sectionList.size());
...
void trackWidget::appendSection(secType _type) {
  ...
  if (!selSection)
    ...
    index =
        selSection->type == anchor
            ? 1
            : inTrack->trackData->getSectionNumber(selSection->sectionData) + 2;
...
void trackWidget::setNames() {
  ...
  for (int i = 1; i < sectionList.size(); ++i) {
    ui->sectionListWidget->topLevelItem(i)->setText(
        1, sectionList[i]->sectionData->sName);
  }
  phantomChanges = oldP;
}
...
void trackWidget::writeNames() {
  ...
  phantomChanges = true;
  for (int i = 1; i < sectionList.size(); ++i) {
    sectionList[i]->sectionData->sName =
        ui->sectionListWidget->topLevelItem(i)->text(1);
  }
  phantomChanges = oldP;
}
...
int trackWidget::getSection(QTreeWidgetItem *item) {
  for (int i = 0; i < sectionList.size(); ++i) {
    if (sectionList[i]->listItem == item)
      return i;
  }
  return -1;
}
...
void trackWidget::setupAnchorFrame() {
  ...
  ui->xBox->setValue((-cos(yaw) * temp.z + sin(yaw) * temp.x +
                      inTrack->trackData->startPos.x) *
                     gloParent->mOptions->getLengthFactor());
  ui->yBox->setValue((temp.y + inTrack->trackData->startPos.y) *
                     gloParent->mOptions->getLengthFactor());
  ui->zBox->setValue(
      (cos(yaw) * temp.x + sin(yaw) * temp.z + inTrack->trackData->startPos.z) *
      gloParent->mOptions->getLengthFactor());
  ui->rBox->setValue(anchor->fRoll);
  ui->pBox->setValue(anchor->getPitch());
  ui->jBox->setValue(inTrack->trackData->startYaw);
  ui->anchorSpeedBox->setValue(anchor->fVel *
                               gloParent->mOptions->getSpeedFactor());
...
void trackWidget::setupStraightFrame() {
  bool oldP = phantomChanges;
  phantomChanges = true;
  ui->straightLengthBox->setValue(selSection->sectionData->fHLength *
                                  gloParent->mOptions->getLengthFactor());
  ...
  ui->straightSpeedBox->setValue(selSection->sectionData->getSpeed() *
                                 gloParent->mOptions->getSpeedFactor());
...
void trackWidget::setupCurvedFrame() {
  bool oldP = phantomChanges;
  phantomChanges = true;
  ui->curvedRadiusBox->setValue(selSection->sectionData->fRadius *
                                gloParent->mOptions->getLengthFactor());
  ...
  ui->curvedSpeedBox->setValue(selSection->sectionData->getSpeed() *
                               gloParent->mOptions->getSpeedFactor());
  ...
  inTrack->mUndoHandler->oldCurveDirection =
      selSection->sectionData->fDirection;
...
void trackWidget::setupAdvFrame() {
  ...
  ui->advancedSpeedBox->setValue(selSection->sectionData->getSpeed() *
                                 gloParent->mOptions->getSpeedFactor());
...
void trackWidget::updateSectionFrame() {
  if (!selSection || selSection->type == anchor)
    return;
  ui->timeLabel->setText(
      QString()
          .number((selSection->sectionData->lNodes.size() - 1) / F_HZ, 'f', 3)
          .append(" s"));
  ui->lengthLabel->setText(
      QString()
          .number(selSection->sectionData->length *
                      gloParent->mOptions->getLengthFactor(),
                  'f', 2)
          .append(" ")
...
void trackWidget::on_yBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->startPos.y =
      (cos(b) * cos(y)) * inTrack->trackData->fHeart +
      arg1 / gloParent->mOptions->getLengthFactor();
...
void trackWidget::on_rBox_valueChanged(double arg1) {
  ...
  if (phantomChanges)
    return;
  inTrack->trackData->anchorNode->setRoll(
      arg1 - inTrack->trackData->anchorNode->fRoll);
  ...
  inTrack->trackData->startPos.y =
      (cos(b) * cos(y)) * inTrack->trackData->fHeart +
      ui->yBox->value() / gloParent->mOptions->getLengthFactor();
  inTrack->trackData->startPos.z =
      (sin(a) * sin(b) * cos(y) - cos(a) * sin(y)) *
          inTrack->trackData->fHeart +
  ...
  bool oldP = phantomChanges;
  phantomChanges = true;
  ui->pitchChangeBox->setValue(inTrack->trackData->anchorNode->fPitchFromLast *
                               F_HZ);
  ui->yawChangeBox->setValue(inTrack->trackData->anchorNode->fYawFromLast *
                             F_HZ);
  phantomChanges = oldP;
...
void trackWidget::on_pBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->anchorNode->changePitch(
      arg1 - inTrack->trackData->anchorNode->getPitch(),
...
void trackWidget::on_anchorSpeedBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->anchorNode->fVel =
      arg1 / gloParent->mOptions->getSpeedFactor();
  inTrack->trackData->anchorNode->fEnergy =
      0.5 * inTrack->trackData->anchorNode->fVel *
          inTrack->trackData->anchorNode->fVel +
      F_G * inTrack->trackData->anchorNode->fPosHearty(
                0.9 * inTrack->trackData->fHeart);
...
void trackWidget::on_straightSpeedCheck_stateChanged(int arg1) {
  ...
  phantomChanges = true;
  ui->straightSpeedBox->setValue(inTrack->trackData->activeSection->getSpeed() *
                                 gloParent->mOptions->getSpeedFactor());
  phantomChanges = false;
  gloParent->updateInfoPanel();
  updateSectionFrame();
...
void trackWidget::on_straightSpeedBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->activeSection->fVel =
      arg1 / gloParent->mOptions->getSpeedFactor();
  inTrack->trackData->updateTrack(inTrack->trackData->activeSection, 0);
  gloParent->updateInfoPanel();
  updateSectionFrame();
...
void trackWidget::on_straightLengthBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->activeSection->rollFunc->setMaxArgument(
      arg1 / gloParent->mOptions->getLengthFactor());
  inTrack->trackData->updateTrack(inTrack->trackData->activeSection, 0);
  if (inTrack->trackData->activeSection->bSpeed) {
    phantomChanges = true;
    ui->straightSpeedBox->setValue(
        inTrack->trackData->activeSection->getSpeed() *
        gloParent->mOptions->getSpeedFactor());
    phantomChanges = false;
  }
  gloParent->updateInfoPanel();
...
void trackWidget::on_curvedSpeedCheck_stateChanged(int arg1) {
  ...
  inTrack->trackData->updateTrack(inTrack->trackData->activeSection, 0);
  phantomChanges = true;
  ui->curvedSpeedBox->setValue(inTrack->trackData->activeSection->getSpeed() *
                               gloParent->mOptions->getSpeedFactor());
  phantomChanges = false;
  gloParent->updateInfoPanel();
  updateSectionFrame();
...
void trackWidget::on_curvedRadiusBox_valueChanged(double arg1) {
  ...
  inTrack->trackData->activeSection->fRadius =
      arg1 / gloParent->mOptions->getLengthFactor();
...
void trackWidget::on_curvedLeadInBox_valueChanged(double arg1) {
  ...
  if (arg1 + inTrack->trackData->activeSection->fLeadOut >
      inTrack->trackData->activeSection->fAngle) {
    arg1 = inTrack->trackData->activeSection->fAngle -
           inTrack->trackData->activeSection->fLeadOut;
    bool oldP = phantomChanges;
    phantomChanges = true;
...
void trackWidget::on_curvedLeadOutBox_valueChanged(double arg1) {
  ...
  if (arg1 + inTrack->trackData->activeSection->fLeadIn >
      inTrack->trackData->activeSection->fAngle) {
    arg1 = inTrack->trackData->activeSection->fAngle -
           inTrack->trackData->activeSection->fLeadIn;
    bool oldP = phantomChanges;
    phantomChanges = true;
...
void trackWidget::on_advancedSpeedCheck_stateChanged(int arg1) {
  ...
  inTrack->trackData->updateTrack(inTrack->trackData->activeSection, 0);
  phantomChanges = true;
  ui->advancedSpeedBox->setValue(inTrack->trackData->activeSection->getSpeed() *
                                 gloParent->mOptions->getSpeedFactor());
  phantomChanges = false;
  gloParent->updateInfoPanel();
  updateSectionFrame();
...
void trackWidget::on_sectionListWidget_customContextMenuRequested(
    const QPoint &pos) {
  QMenu *menu = new QMenu(this);
  menu->setAttribute(Qt::WA_DeleteOnClose);
  ...
  if (ui->sectionListWidget->itemAt(pos) == NULL) {
    ...
    menu->addAction("Append Geometric Section", this,
                    SLOT(appendGeometricSec()));
...
void trackWidget::updateAnchorGeometrics() {
  glm::vec3 forceVec = glm::vec3(0, 1, 0) +
                       inTrack->trackData->anchorNode->forceNormal *
                           inTrack->trackData->anchorNode->vNorm +
                       inTrack->trackData->anchorNode->forceLateral *
                           inTrack->trackData->anchorNode->vLat;

  glm::vec3 pitchVec =
      (float)cos(inTrack->trackData->anchorNode->fRoll * F_PI / 180) *
          inTrack->trackData->anchorNode->vNorm -
      (float)sin(inTrack->trackData->anchorNode->fRoll * F_PI / 180) *
          inTrack->trackData->anchorNode->vLat;
  glm::vec3 yawVec =
      (float)sin(inTrack->trackData->anchorNode->fRoll * F_PI / 180) *
          inTrack->trackData->anchorNode->vNorm +
  ...
  inTrack->trackData->anchorNode->fPitchFromLast =
      glm::dot(forceVec, pitchVec) / inTrack->trackData->anchorNode->fVel *
      1.8 / F_PI;
  inTrack->trackData->anchorNode->fYawFromLast =
      glm::dot(forceVec, yawVec) / inTrack->trackData->anchorNode->fVel * 1.8 /
      F_PI;
}
...
void trackWidget::on_pitchChangeBox_valueChanged(double arg1) {
  ...
  float forceAngle =
      sqrt(temp * temp * anchor->fYawFromLast * anchor->fYawFromLast +
           anchor->fPitchFromLast * anchor->fPitchFromLast); // deltaAngle;
  float dirFromLast = glm::atan(anchor->fYawFromLast, anchor->fPitchFromLast) -
                      TO_RAD(anchor->fRoll);
  ...
  if (fabs(forceAngle) < std::numeric_limits<float>::epsilon()) {
    ...
    forceVec =
        ...
        (float)((anchor->fVel * anchor->fVel) /
                (9.80665 * anchor->fVel / forceAngle * 0.18f / F_PI)) *
            glm::normalize(glm::vec3(glm::rotate(dirFromLast, -anchor->vDir) *
                                     glm::vec4(-anchor->vNorm, 0.f)));
  }
...
void trackWidget::on_sectionListWidget_itemChanged(QTreeWidgetItem *item,
                                                   int column) {
  Q_UNUSED(item);
  if (phantomChanges)
    return;
